import { PasetoPayloadInvalid } from "./errors.ts";
 ...
 * @see https://github.com/paseto-standard/paseto-spec/blob/master/docs/02-Implementation-Guide/01-Payload-Processing.md#enforcing-maximum-depth-without-parsing-the-json-string
 */
export function assertJsonStringSize(
  json: string,
  { maxDepth = 10, maxKeys = 100 }: { maxDepth: number; maxKeys: number },
): boolean {
  // assert json is a string
  if (typeof json !== "string") {
    throw new PasetoPayloadInvalid(
      `JSON string must be a string (got ${typeof json}))`,
    );
  }
  if (maxDepth || maxKeys) {
    ...
    if (maxDepth && maxDepth > 0 && depth > maxDepth) {
      throw new PasetoPayloadInvalid(
        `JSON string exceeds maximum depth of ${maxDepth}`,
      );
    }
    if (maxKeys && maxKeys > 0 && keys > maxKeys) {
      throw new PasetoPayloadInvalid(
        `JSON string exceeds maximum number of keys of ${maxKeys}`,
      );
    }
  }
  return true;
 ...
 */
export function returnPossibleJson(
