import { KEY_MAGIC_BYTES } from "../../../lib/utils/magic.ts";
import { concat } from "../../../lib/utils/uint8array.ts";
import { decrypt } from "../../../lib/v4/decrypt.ts";
import { isObject } from "../../../lib/utils/validate.ts";
import { expect } from "@std/expect";
import { PasetoTokenInvalid } from "../../../lib/utils/errors.ts";
...
Deno.test("it passes test vectors", () => {
  ...
  for (const vector of vectors) {
    if (vector.key) {
      ...
      if (vector["expect-fail"]) {
        try {
          decrypt(key, vector.token, {
            validatePayload: false,
            assertion: vector["implicit-assertion"]
              ? vector["implicit-assertion"]
              : undefined,
          });
        } catch (error) {
          expect(error).toBeInstanceOf(PasetoTokenInvalid);
        }
      } else {
        ...
        expect(payloadStr).toEqual(vector.payload);

        if (vector.footer) {
          if (isObject(decrypted.footer)) {
            const footerStr = JSON.stringify(decrypted.footer);
            expect(footerStr).toEqual(vector.footer);
          } else {
            expect(decrypted.footer).toEqual(vector.footer);
          }
        }
      }
    }
